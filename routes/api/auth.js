...
router.post('/',[
 ...
 async (req,res)=>{
    ...
    try {
        ...
        if(!isMatch){
            return res.status(400).json({errors : [{msg: 'Invalid Credentials'}] }); // It is safer to place same error message to bad user and bad password since people can't assume which one of them is wrong,  making the app more secure
        }
